...
export interface UnifiedEditOperations {
  access(path: string, mode?: number): Promise<void>
  mkdir(path: string, options: { recursive: true }): Promise<unknown>
  readFile(path: string, encoding: 'utf-8'): Promise<string>
  realpath(path: string): Promise<string>
  rename(source: string, destination: string): Promise<void>
  stat(path: string): Promise<{ dev: number | bigint | string; ino: number | bigint | string }>
  unlink(path: string): Promise<void>
  writeFile(path: string, content: string, encoding: 'utf-8'): Promise<void>
}

export type UnifiedEditOperationsProvider = (
  cwd: string,
  context?: ExtensionContext,
) => Promise<UnifiedEditOperations | undefined>

export const localUnifiedEditOperations: UnifiedEditOperations = {
...
async function readExistingNormalized(
  path: string,
  absolutePath: string,
  fs: UnifiedEditOperations,
): Promise<string> {
  try {
    return normalizeToLf(stripBom(await fs.readFile(absolutePath, 'utf-8')).text)
  } catch (err) {
...
async function maybeReadNormalized(
  absolutePath: string,
  fs: UnifiedEditOperations,
): Promise<string | null> {
  try {
...
async function pathsReferenceSameFile(
  a: string,
  b: string,
  fs: UnifiedEditOperations,
): Promise<boolean> {
...
async function buildPatchPlan(
  ...
  const store = createSnapshotStore(cwd, (_path, absolutePath) =>
    maybeReadNormalized(absolutePath, fs),
  )
  let moveOrder = 0
  ...
  for (const op of operations) {
    ...
    if (
      ...
      (await canonicalMutationPath(destination.absolutePath, fs)) ===
        (await canonicalMutationPath(snapshot.absolutePath, fs)) ||
      (await pathsReferenceSameFile(destination.absolutePath, snapshot.absolutePath, fs))
    ) {
...
async function buildPreviewPlan(
  ...
  if (!argsComplete && isPatchLikePayload(text) && !isPatchPayload(text)) {
    return buildPatchPlan(patchTextForPreview(text), cwd, fs)
  }
  return buildPatchPlan(text, cwd, fs)
}
...
async function checkCanCreatePath(
  absolutePath: string,
  fs: UnifiedEditOperations,
): Promise<void> {
  ...
  while (true) {
    try {
      await fs.access(dir, constants.W_OK | constants.X_OK)
      return
    } catch (err) {
...
async function preflightPlan(
  ...
  for (const change of plan.changes) {
    ...
    if (change.kind === 'add') {
      await checkCanCreatePath(change.absolutePath, fs)
      continue
    }
    ...
    if (change.kind === 'move') {
      ...
      await fs.access(change.absolutePath, sourceAccess)
      await fs.access(dirname(change.absolutePath), constants.W_OK | constants.X_OK)
      if (change.destinationOldText === null)
        await checkCanCreatePath(change.destinationAbsolutePath, fs)
      else
        await fs.access(
          dirname(change.destinationAbsolutePath),
          constants.W_OK | constants.X_OK,
        )
      continue
    }
    await fs.access(change.absolutePath, constants.R_OK | constants.W_OK)
  }
}
...
async function readFileForMutation(
  ...
  const { bom, text } = stripBom(await fs.readFile(absolutePath, 'utf-8'))
...
async function applyUpdateChange(
  ...
  return withFileMutationQueue(change.absolutePath, async () => {
    throwIfAborted(signal)
    const file = await readFileForMutation(change.absolutePath, fs)
    throwIfAborted(signal)
...
async function applyAddChange(
  ...
  return withFileMutationQueue(change.absolutePath, async () => {
    throwIfAborted(signal)
    const existing = await maybeReadNormalized(change.absolutePath, fs)
    if (existing !== null) throw new Error(`Could not add ${change.path}: file already exists.`)
    await fs.mkdir(dirname(change.absolutePath), { recursive: true })
    await fs.writeFile(change.absolutePath, change.newText, 'utf-8')
...
async function applyDeleteChange(
  ...
  return withFileMutationQueue(change.absolutePath, async () => {
    throwIfAborted(signal)
    await fs.access(change.absolutePath, constants.R_OK | constants.W_OK)
    const current = await readExistingNormalized(change.path, change.absolutePath, fs)
...
async function withMutationQueues<T>(
  ...
  while (true) {
    const canonicalPaths = await Promise.all(paths.map(path => canonicalMutationPath(path, fs)))
    ...
    async function run(index: number, heldPaths: Set<string>): Promise<T> {
      ...
      const queuePath = await canonicalMutationPath(queuePaths[index], fs)
...
async function applyMoveChange(
  ...
  return withMutationQueues([change.absolutePath, destinationAbsolutePath], fs, async () => {
    throwIfAborted(signal)
    const sourceAccess =
      change.oldText === change.newText ? constants.R_OK : constants.R_OK | constants.W_OK
    const source = await readFileForMutation(change.absolutePath, fs, sourceAccess)
    if (source.content !== change.oldText) {
      throw new Error(`Could not move ${change.path}: source file changed since preflight.`)
    }
    const currentDestination = await maybeReadNormalized(destinationAbsolutePath, fs)
    if (currentDestination !== destinationOldText) {
      throw new Error(
        `Could not move ${change.path}: destination ${destinationPath} changed since preflight.`,
      )
    }
    if (await pathsReferenceSameFile(change.absolutePath, destinationAbsolutePath, fs)) {
      throw new Error(
        `Could not move ${change.path}: destination ${destinationPath} changed since preflight.`,
      )
    }

    await fs.mkdir(dirname(destinationAbsolutePath), { recursive: true })
    await fs.rename(change.absolutePath, destinationAbsolutePath)
    try {
...
async function applyPlan(
  ...
  for (const change of plan.changes) {
    throwIfAborted(signal)
    const details = await appliers[change.kind](change, fs, signal)
    files.push({
...
function requestUnifiedEditPreview(
  ...
  cwd: string,
  argsComplete: boolean,
  getOperations: UnifiedEditOperationsProvider,
  invalidate: () => void,
): void {
  ...
  void getOperations(cwd)
    .then(fs => (fs ? buildPreviewPlan(text, cwd, argsComplete, fs) : undefined))
    .then((plan): Preview | undefined => (plan ? previewForPlan(plan) : undefined))
...
export default function unifiedEditExtension(
  pi: ExtensionAPI,
  getOperations: UnifiedEditOperationsProvider = async () => localUnifiedEditOperations,
) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params: UnifiedEditParams, signal, _onUpdate, ctx) {
      ...
      const fs = await getOperations(ctx.cwd, ctx)
      if (!fs) throw new Error('edit filesystem is not available.')
      const plan = await buildPatchPlan(text, ctx.cwd, fs)
      try {
        await preflightPlan(plan, fs, signal)
      } catch (err) {
        throw new Error(`Preflight failed before mutating files.\n${getErrorMessage(err)}`)
      }
      const details = await applyPlan(plan, fs, signal)
