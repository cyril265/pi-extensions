...
export const EnclaveFileConfig = v.object({
	// Legacy setting retained so existing generated configs still parse. Activation is session-only.
...
export function mergeConfigs(layers: EnclaveFileConfig[]): EnclaveFileConfig {
	...
	for (const layer of layers) {
		// Preserve the legacy value so the extension can explain that it is ignored.
...
/** Create the disabled-by-default project config. Returns true if the file was created. */
