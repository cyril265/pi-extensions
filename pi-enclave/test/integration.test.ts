...
describe("initProjectConfig", () => {
	it("creates a disabled-by-default project config from template", () => {
		...
		expect(content).not.toMatch(/^enabled\s*=\s*true/m);
		expect(loadConfig(projectDir).merged.enabled).toBeUndefined();
	});
...
describe("collectConfigFiles with drop-ins", () => {
	...
	it("reports config sources accurately", () => {
		...
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), "packages = []\n");
	...
	it("uses the last configured image", () => {
		...
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), 'image = "project:latest"\n');
...
describe("mount path resolution", () => {
	it("expands ~ to HOME in mount paths", () => {
		...
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), '[[mounts]]\npath = "~/dev/.jj"\n');
		const { merged } = loadConfig(projectDir);
	...
	it("expands ~ in bare string mounts", () => {
		...
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), 'mounts = ["~/dev/.jj", "~/dev/.git"]\n');
		const { merged } = loadConfig(projectDir);
	...
	it("resolves relative paths against cwd", () => {
		...
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), 'mounts = ["../.jj", "../.git"]\n');
		const { merged } = loadConfig(projectDir);
		expect(merged.mounts).toHaveLength(2);
	...
	it("resolves nested relative paths against cwd", () => {
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), 'mounts = ["../shared/data"]\n');
		const { merged } = loadConfig(projectDir);
	...
	it("leaves absolute paths unchanged", () => {
		const projectDir = join(tmpDir, "project");
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "enclave.toml"), 'mounts = ["/tmp/shared"]\n');
		const { merged } = loadConfig(projectDir);
