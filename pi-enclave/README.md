...
This local fork targets `@earendil-works/pi-coding-agent` and combines enclave execution with the unified patch `edit` tool.
...
- `.pi/enclave.toml` — project config, disabled by default

Enable the enclave explicitly for the current session:

```
/enclave on
```

Once enabled, all tools (bash, read, write, edit) execute inside the VM. Activation is not persisted: each new or reloaded pi session starts with the enclave disabled.

### Unified edit

The `edit` tool accepts one `*** Begin Patch` / `*** End Patch` payload containing any number of add, update, move, and delete operations. Planning, previews, and mutations use the enclave filesystem while isolation is active. When the enclave is off, the same tool uses the host filesystem like the other local tools.
...
```toml
# .pi/enclave.toml — allow all GitHub operations when the enclave is enabled
[hosts."api.github.com"]
unmatched = "allow"
```
...
| `/enclave init` | Create project and global config files (disabled by default) |
...
| `/enclave add <package>` | Search for and install an Alpine package while enabled |
