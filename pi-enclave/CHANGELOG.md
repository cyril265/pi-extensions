...
- Start every new or reloaded pi session with the enclave disabled. Activation is now in-memory only through `/enclave on`; legacy `enabled = true` config and `/enclave add` no longer activate it implicitly.
- Combine unified patch editing with enclave-backed filesystem operations under one `edit` tool registration.
- Update `@earendil-works/gondolin` from 0.6 to 0.12.
- Target `@earendil-works/pi-coding-agent` 0.80 and its current session lifecycle.
- Make the copied package build and test independently of the yapp monorepo.
