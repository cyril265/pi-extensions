...
export async function executeSubagents(
  ...
  onUpdate: AgentToolUpdateCallback<SubagentResultDetails | undefined> | undefined,
  ...
  const modelContext = ctx as {
    model?: { provider: string; id: string }
  }
  const callerModelInfo = modelContext.model
  ...
  const resolvedAgents = requestedAgents.map(agent => {
    ...
    const effectiveModel =
      resolveEffectiveModel(suppliedModel, modelAliases) || callerModel
    if (!effectiveModel) {
  ...
  const liveAgents: SubagentResultDetails['agents'] = resolvedAgents.map(agent => ({
    name: agent.name,
    thinking: agent.thinking,
  ...
  const preparedAgents = agentsWithSessionPaths.map(agent => {
    if (!agent.forkParent) {
      if (agent.sessionPath && readForkMetadata(agent.sessionPath)) {
        throw new Error(
    ...
    const sessionPath = agent.sessionPath
    const existingMetadata = readForkMetadata(sessionPath)
    const sessionExists = fs.existsSync(sessionPath)
    if (sessionExists !== !!existingMetadata) {
      throw new Error(`Incomplete forked session state for "${agent.name}" at ${sessionPath}`)
    }
    ...
    try {
      writeForkMetadata(sessionPath, expectedMetadata)
    } catch (error) {
      fs.unlinkSync(sessionPath)
      throw error
    }
    return { ...agent, promptCacheKey: inheritsParentCache ? expectedMetadata.promptCacheKey : undefined }
  })
  ...
  if (!runInChildProcesses) {
    try {
      ...
      herdrWarning = error instanceof Error ? error.message : String(error)
  ...
  if (runInChildProcesses) {
    ...
    if (signal?.aborted) {
      const sessions = resolvedAgents.map(agent => `${agent.name}: ${agent.sessionKey}`).join(', ')
      throw new Error(
