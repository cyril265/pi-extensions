...
A distributable Pi extension package that replays **OpenAI Responses compaction V2** checkpoints without patching Pi core.

This package preserves the encrypted compaction item returned by the Responses SSE stream, stores it in Pi compaction entry details, and rewrites later supported OpenAI Responses requests to:
...
- **Minimum Pi version:** `@earendil-works/pi-coding-agent >= 0.79.4`
...
1. `session_before_compact`
   ...
   - call the normal Responses endpoint with the `remote_compaction_v2` feature and a trailing `compaction_trigger`
   - capture the raw `compaction_summary` output item and persist its canonical encrypted checkpoint
...
│   ├── compact-client.ts       # Responses V2 SSE compaction client
